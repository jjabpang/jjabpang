...
- [짭팡 (jjab_pang)](#짭팡-(jjab_pang))
- [JjabPang](#jjabpang)
- [목차](#목차)
    * [개발 환경](#개발-환경)
        + [백엔드 (박상준)](#백엔드-(박상준))
        + [프론트엔드 (김신협, 이주상)](#프론트엔드-(김신협,-이주상))
    * [사용 기술](#사용-기술)
        + [백엔드](#백엔드)
    ...
    * [핵심 기능(기능 개발 중)](#핵심-기능(릴리즈-전))
        ...
        + [github 액션 + AWS CODE DEPLOY + AWS EC2](#aws)
            - [이미지와 썸네일 삽입시는 ~~깃허브~~ aws s3 이미지 서버로](#s3)
        + [스웨거 사용](#스웨거-사용)
        + [기본적인 게시물 CRUD](#기본적인-게시물-CRUD)
        + [카테고리 깊이 구현 및 추후 깊이관련 기능 추가예정](#카테고리-깊이-구현-및-추후-깊이관련-기능-추가예정)
        + [에러처리](#에러처리)
        + [Qdsl 동적 정렬 및 동적 조건](#Qdsl-동적-정렬-및-동적-조건)
        + [CI/CD 무중단 배포](#CI/CD-무중단-배포)
        + [intellij .http 사용(api호출관련)](#intellij-.http-사용(api호출관련))
...
테스트 코드를 통한 기능 추가 검증 진행 중
테스트 코드를 짜서 진행 하면 기능 개발 시간에 문제가 생길까봐.. 테스트 코드를 작성하면서 기능 개발은 하지 못했다.

원래 프론트엔드와 동시에 작업예정이라 바빴으나.. 일정의 변경으로 백엔드만 먼저 기능 개발이 진행되었다.

### 에러 전역 설정

![img_11.png](img_11.png)
(2022-08-21)  
REST API 에러의 전역 설정으로 json데이터로 모든 에러를 뽑아낼 수 있도록함.
혹여나 처리하지 못한 예외에서 벗어날 수 있도록 합니다.

### 스프링 배치 추가중

(2022-08-23)

정기배송이 존재하는 경우 어떻게 대처할 것인가?   
에 대해서 생각해 보았습니다.
추가중입니다.

참고 - [https://www.youtube.com/watch?v=wy99cPHlMlA](https://www.youtube.com/watch?v=wy99cPHlMlA)
...
### github 액션 + AWS CODE DEPLOY + AWS EC2<a id="aws"></a>

배포 자동화를 사용하여 개발자가 코드를 푸쉬 혹은 `PR` 을 신청할 때마다 EC2 서버에 반영되도록 하였습니다.

#### 이미지와 썸네일 삽입시는 ~~깃허브~~ aws s3 이미지 서버로 <a id="s3"></a>
...
### intellij .http 사용(api호출관련)

![img_8.png](img_8.png)
![img_9.png](img_9.png)

POSTMAN을 사용하려고 하였으나.. 인텔리제이에 맛있어 보이는 기능이 있더라..
전부터 이게 뭔가.. 싶었는데..

개발/운영 환경에서 API요청을 간단하게 테스트해볼때 사용할 수 있는 기능이다.

POSTMAN보다 1) Code highlighting ,2) 코드 자동 완성 , 3) 직관적인 HttpRequest 구조 면에서 뛰어나다고 생각한다.

물론 세부적인 설정면에서 POSTMAN에 밀리는 건 단점이라고 생각된다.
